...
class ExperimentSet(gsim.AbstractExperimentSet):
    ...
    def experiment_2001(args):
        ...
        def generate_dataset(num_maps_train=100,
            ...
            filename = os.path.join(
                datafolder,
                f"grid_{grid.num_points_y}_x_{grid.num_points_x}"
    ...
    def experiment_20051(args):
        ...
        weights_file_string = (weightfolder +"100_meas_nn_arch_20_alpha=0_epochs=30_samp_scale=0.7_out_same=False")



        # 2. 先让模型“跑”一下虚拟数据，触发其内部变量创建
        # dummy_input = tf.zeros([1, 32, 32, 1])  # 根据你的实际输入形状修改
        # model(dummy_input)  # 或者 model.build((None, 32, 32, 1))
    ...
    def experiment_9005(args):
        ...
        # 1. 构建网格（与2001数据生成时保持一致）
        grid = RectangularGrid(
            gridpoint_spacing=3,
            num_points_x=32,  # 与数据生成时相同
            num_points_y=32,  # 与数据生成时相同
            height=20)
        ...
        def train_network(num_maps_train=None, num_blocks_per_map=20,
            ...
            with open(loss_file_string, 'wb') as outfile:
                pickle.dump(dict_history, outfile)
        ...
        # 注意，这里需要确保传入的参数与生成数据时一致
        G = train_network(num_maps_train=5000, num_blocks_per_map=50,
